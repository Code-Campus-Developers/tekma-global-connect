...
export function SiteHeader() {
  ...
  return <header className="sticky top-0 z-50 border-b border-border/70 bg-background/95 backdrop-blur-xl">
    <div className="mx-auto grid h-20 max-w-7xl grid-cols-[auto_1fr_auto] items-center gap-6 px-5 lg:px-8">
      <Link to="/" aria-label="TEKMA Global Partners home" className="flex shrink-0 items-center gap-1"><span className="hidden max-w-[9rem] text-[10px] font-extrabold leading-none tracking-[0.06em] text-navy sm:inline sm:max-w-[11rem] sm:text-xs">TEKMA<br/>GLOBAL<br/>PARTNERS</span><img src={logo.url} alt="TEKMA Global Partners Limited" width={72} height={69} className="h-16 w-auto object-contain" /></Link>
    ...
    {open && <div className="fixed inset-0 z-[60] min-h-dvh overflow-y-auto bg-navy text-primary-foreground lg:hidden">
      <div className="flex items-center justify-between border-b border-primary-foreground/15 p-5"><div className="flex items-center gap-1"><span className="max-w-[9rem] text-[10px] font-extrabold leading-none tracking-[0.06em] text-white">TEKMA<br/>GLOBAL<br/>PARTNERS</span><img src={logo.url} alt="TEKMA Global Partners Limited" className="h-16 w-auto" width={72} height={69}/></div><Button variant="ghost" size="icon" aria-label="Close menu" onClick={()=>setOpen(false)} className="min-h-11 min-w-11 text-primary-foreground"><X/></Button></div>
