...
export function SiteHeader() {
  ...
  return <header className="sticky top-0 z-50 border-b border-border/70 bg-background/95 backdrop-blur-xl">
    ...
    {open && <div className="fixed inset-0 z-[60] min-h-dvh overflow-y-auto bg-navy text-primary-foreground lg:hidden">
      <div className="flex items-center justify-between border-b border-primary-foreground/15 p-5"><div className="flex items-center gap-1.5"><img src={logo.url} alt="TEKMA Global Partners Limited" className="h-16 w-auto" width={72} height={69}/><span className="whitespace-nowrap text-xs font-extrabold leading-none tracking-[0.04em] text-white sm:text-sm">TEKMA GLOBAL PARTNERS</span></div><Button variant="ghost" size="icon" aria-label="Close menu" onClick={()=>setOpen(false)} className="min-h-11 min-w-11 text-primary-foreground"><X/></Button></div>
