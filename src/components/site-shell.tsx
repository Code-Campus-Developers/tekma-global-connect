...
export function SiteHeader() {
  ...
  return <header className="sticky top-0 z-50 border-b border-border/70 bg-background/95 backdrop-blur-xl">
    <div className="mx-auto flex h-20 max-w-7xl items-center gap-4 px-5 lg:grid lg:grid-cols-[auto_1fr_auto] lg:gap-6 lg:px-8">
      <Link to="/" aria-label="TEKMA Global Partners home" className="flex min-w-0 flex-1 items-center gap-2 lg:flex-none"><img src={logo} alt="TEKMA Global Partners Limited" width={72} height={69} className="h-12 w-auto shrink-0 object-contain lg:h-14" /><span className="truncate text-base font-extrabold leading-none tracking-[0.04em] text-navy sm:text-xl xl:text-2xl">TEKMA GLOBAL PARTNERS</span></Link>
      <nav aria-label="Primary navigation" className="ml-auto hidden items-center justify-center gap-5 lg:flex">
      ...
      <Button asChild className="hidden min-h-11 whitespace-nowrap rounded-none bg-navy px-4 text-xs font-bold uppercase tracking-[0.12em] hover:bg-petroleum lg:inline-flex"><Link to="/opportunities">Discuss an opportunity</Link></Button>
      <Button variant="ghost" size="icon" aria-label="Open menu" onClick={()=>setOpen(true)} className="min-h-11 min-w-11 shrink-0 lg:hidden"><Menu /></Button>
    </div>
    {open && <div className="fixed inset-0 z-[60] min-h-dvh overflow-y-auto bg-navy text-primary-foreground lg:hidden">
      <div className="flex items-center justify-between gap-4 border-b border-primary-foreground/15 p-5"><div className="flex min-w-0 items-center gap-2"><img src={logo} alt="TEKMA Global Partners Limited" className="h-14 w-auto shrink-0" width={72} height={69}/><span className="whitespace-nowrap text-lg font-extrabold leading-none tracking-[0.04em] text-white sm:text-2xl">TEKMA GLOBAL PARTNERS</span></div><Button variant="ghost" size="icon" aria-label="Close menu" onClick={()=>setOpen(false)} className="min-h-11 min-w-11 shrink-0 text-primary-foreground"><X/></Button></div>
...
export function SiteFooter() {
  return <footer className="bg-navy text-primary-foreground"><div className="mx-auto max-w-7xl px-5 py-16 lg:px-8"><div className="grid gap-12 border-b border-primary-foreground/15 pb-12 md:grid-cols-[1.3fr_1fr_1fr]">
    <div><img src={logo} alt="TEKMA Global Partners Limited" className="h-24 w-auto" width={100} height={96}/><p className="mt-5 max-w-sm text-sm leading-7 text-primary-foreground/70">Global brokerage and consultancy across energy, assets, maritime and commercial real estate.</p></div>
